...
<section>
  ...
  <div class="dist">
${dist.map(([k, n]) => `    <span class="d-${k.toLowerCase()}" style="width:${(n / g.length * 100).toFixed(2)}%">${k} ${n}</span>`).join('\n')}
  </div>
  <div class="dist-key">
    <span>A · clean on every check</span><span>B · minor flag</span><span>D · profitable but compromised</span><span>F · not copyable</span>
  </div>
</section>
...
<section>
  ...
  <div class="callout">
    ...
    <h3>The most profitable wallet on this list earns ${'AEIOU'.includes(botMost.grade) ? 'an' : 'a'}n ${botMost.grade}.</h3>
...
<section>
  <h2>Ratings</h2>
  <p class="sec-note">Sorted by score. ROI is shown two ways on purpose: staked-weighted and equal-weighted. When the two disagree in sign, the record is carried by a handful of large bets rather than a repeatable process.</p>
  <div class="tablewrap">
    <table>
      <thead><tr>
