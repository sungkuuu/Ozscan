...
// per-wallet page cap (mega-bots can't eat the run), DB size ceiling 74GB.
...
const DB_CEILING = 74 * 1024 ** 3;   // volume grown to 80GB on 8/25 night; 6GB headroom
...
while (Date.now() - t0 < TIME_BUDGET_MS) {
  ...
  if (Number(sz.s) > DB_CEILING) { console.log('DB ceiling 74GB reached — stopping. Review before continuing.'); process.exit(3); }
