...
// per-wallet page cap (mega-bots can't eat the run), DB size ceiling 46GB.
...
const DB_CEILING = 46 * 1024 ** 3;   // volume grown to 50GB on 8/25 evening; 4GB headroom
...
while (Date.now() - t0 < TIME_BUDGET_MS) {
  ...
  if (Number(sz.s) > DB_CEILING) { console.log('DB ceiling 46GB reached — stopping. Review before continuing.'); process.exit(3); }
